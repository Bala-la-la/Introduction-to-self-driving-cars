...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            robot_state[0] = self._current_x + 2 * np.cos(self._current_yaw)
            robot_state[1] = self._current_y + 2 * np.sin(self._current_yaw)

            # Find the nearest waypoint
            for i in range(len(self._waypoints)):
                dist1 = np.sqrt(
                    (self._waypoints[i][0] - robot_state[0]) ** 2 + (self._waypoints[i][1] - robot_state[1]) ** 2)
                if dist1 < dist:
                    dist = dist1
                    idx = i

            if self._waypoints[idx][1] > robot_state[1]:
                laterr = self._waypoints[idx][1] - robot_state[1]
            else:
                laterr = -(self._waypoints[idx][1] - robot_state[1])

            if idx < len(self._waypoints) - 6:
                idy = idx+5
            else:
                idy = len(self._waypoints) - 1

            theta_fai = self.steer_output_p - np.arctan((self._waypoints[idy][1] - robot_state[1]) / (self._waypoints[idy][0] - robot_state[0]))
            if theta_fai > np.pi:
                ...
                theta_fai = theta_fai
            theta_y = np.arctan(5*laterr/self._current_speed)
        ...
        self.vars.steer_output_p = steer_output # Store forward speed to be used in next step
